...
class NestJSAnalyzer {
    ...
    analyzeResolverFile(file) {
        ...
        (0, traverse_1.default)(file.ast, {
            ClassDeclaration: (path) => {
                ...
                const hasResolverDecorator = this.hasDecorator(path.node.decorators || [], this.options.decorators.resolver);
    ...
    hasDecorator(decorators, decoratorNames) {
    ...
    findDecorator(decorators, decoratorNames) {
    ...
    analyzeDependencies(method, file) {
        const dependencies = [];
        // Traverse method body to find service calls
        (0, traverse_1.default)(method, {
    ...
    inferGrpcCall(serviceName, methodName) {
    ...
    inferServiceName(name, packageName, protoPath) {
