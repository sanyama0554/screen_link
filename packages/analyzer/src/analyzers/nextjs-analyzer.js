...
class NextJSAnalyzer {
    ...
    analyzeAppRouter(files) {
        const screens = [];
        const appPath = this.options.appPath;
        // Find app directory files
        const appFiles = files.filter(file => file.path.includes('/app/') &&
    ...
    analyzePagesRouter(files) {
        const screens = [];
        // Find pages directory files
        const pageFiles = files.filter(file => file.path.includes('/pages/') &&
    ...
    extractAppRouterRoute(filePath, appPath) {
    ...
    extractPagesRouterRoute(filePath) {
